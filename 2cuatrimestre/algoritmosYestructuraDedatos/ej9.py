	...
	for i in range(naranjas):
		...
		if peso_naranja > 200 and peso_naranja < 300:
			if (naranjas-i) > 100:
				...
				peso_restante = peso_restante + peso_naranja
		else:
			jugo = jugo + 1

	return consumo, jugo,total_peso,peso_restante,restantes

def calcular_cajones(consumo):
	cajones = consumo/MAX_NARANJAS
	consumo_incompleto = consumo%MAX_NARANJAS
...
def main():
	...
	consumo,jugo,total_peso,peso_restante,restantes = verificar_peso(naranjas)
	calcular_cajones(consumo)
